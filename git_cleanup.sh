...
cd Dropbox/Programacion/Proyectos/MarioVarona/v2
...
for folder in ${KEEPLATEST[@]}; do
  echo "stashing the latest version of $folder"
	rsync -az Dropbox/Programacion/Proyectos/MarioVarona/v2/${PROJ}-shrinking/$folder/ Dropbox/Programacion/Proyectos/MarioVarona/v2/${PROJ}-stash/$folder
done
...
for folder in ${KEEPLATEST[@]}; do
  echo "restoring from stashed copy of $folder"
	rsync -az Dropbox/Programacion/Proyectos/MarioVarona/v2/${PROJ}-stash/$folder/ Dropbox/Programacion/Proyectos/MarioVarona/v2/${PROJ}-addback/$folder
	git add . --all
